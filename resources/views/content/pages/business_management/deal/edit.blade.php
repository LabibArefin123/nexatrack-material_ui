...
@section('content')
    ...
    <div class="card">
        <div class="card-body">
            <form action="{{ route('deals.update', $deal->id) }}" method="POST" class="p-4">
                @csrf
                @method('PUT')
                <div class="row">
                    {{-- Name --}}
                    ...
                    <div class="form-group col-md-6">
                        ...
                        <select name="deal_stage" id="deal_stage"
                            ...
                            @foreach (['new', 'create_stage', 'invoice', 'in_progress', 'final_invoice', 'deal_won', 'deal_lost', 'analyze_failure'] as $stage)
                                <option value="{{ $stage }}"
                                    {{ old('deal_stage', $deal->deal_stage) == $stage ? 'selected' : '' }}>
                    ...
                    <div class="form-group col-md-4">
                        ...
                        <input type="text" name="amount" id="amount" value="{{ old('amount', $deal->amount) }}"
                        ...
                        <select name="currency" id="currency" class="form-control @error('currency') is-invalid @enderror">
                            ...
                            @foreach (['BDT', 'USD', 'EUR', 'GBP', 'INR'] as $currency)
                                <option value="{{ $currency }}"
                                    {{ old('currency', $deal->currency) == $currency ? 'selected' : '' }}>
                                    {{ $currency }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="form-group col-md-6">
                        ...
                        <input type="date" name="end_date" id="end_date" value="{{ old('end_date', $deal->end_date) }}"
                        ...
                        <select name="client_option" id="client_option"
                            ...
                            @foreach ($clients as $client)
                                <option value="{{ $client->name }}"
                                    {{ old('client_option', $deal->client_option) == $client->name ? 'selected' : '' }}>
                                    {{ $client->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="form-group col-md-6">
                        ...
                        <select name="company_option" id="company_option"
                            ...
                            @if ($deal->client_option && isset($companiesByClient[$deal->client_option]))
                                @foreach ($companiesByClient[$deal->client_option] as $company)
                                    <option value="{{ $company }}"
                                        {{ old('company_option', $deal->company_option) == $company ? 'selected' : '' }}>
                                        {{ $company }}
                                    </option>
                                @endforeach
                            @endif
                    ...
                    <div class="form-group col-md-6">
                        ...
                        <select name="deal_type" id="deal_type"
                            ...
                            @foreach (['sales', 'integrated_sales', 'merchandise_sales', 'services', 'after_sales_service'] as $type)
                                <option value="{{ $type }}"
                                    {{ old('deal_type', $deal->deal_type) == $type ? 'selected' : '' }}>
                    ...
                    <div class="form-group col-md-6">
                        ...
                        <select name="source" id="source" class="form-control @error('source') is-invalid @enderror">
                            ...
                            @foreach (['call', 'email', 'website', 'advertising', 'existing_client', 'by_recommendation', 'show_or_exhibition', 'crm_form', 'callback', 'sales_boost', 'online_store', 'other'] as $source)
                                <option value="{{ $source }}"
                                    {{ old('source', $deal->source) == $source ? 'selected' : '' }}>
                    ...
                    <div class="form-group col-md-6">
                        ...
                        <textarea name="comment" id="comment" class="form-control summernote">{{ old('comment', $deal->comment) }}</textarea>
                        @error('comment')
                            <span class="text-danger small">{{ $message }}</span>
                        @enderror
                    </div>
                    ...
                    <div class="form-group col-md-6">
                        ...
                        <textarea name="source_information" id="source_information" class="form-control summernote">{{ old('source_information', $deal->source_information) }}</textarea>
...
@section('js')
    ...
    <script>
        $(function() {
            ...
            @if (old('comment', $deal->comment))
                $('#comment').summernote('code', {!! json_encode(old('comment', $deal->comment)) !!});
            @endif

            @if (old('source_information', $deal->source_information))
                $('#source_information').summernote('code', {!! json_encode(old('source_information', $deal->source_information)) !!});
            @endif
        });
    </script>
    <script>
        ...
        function populateCompanies(clientName) {
            ...
            companies.forEach(company => {
                ...
                if (company === "{{ old('company_option', $deal->company_option) }}") {
        ...
        if (clientSelect.value) {
            populateCompanies(clientSelect.value);
        }
