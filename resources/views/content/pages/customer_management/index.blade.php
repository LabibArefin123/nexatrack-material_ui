                    ...
                    <div class="table-responsive mt-3">
                        <table id="customers-table" class="table table-bordered table-hover text-nowrap mb-0">
                            ...
                            <tbody id="customer-body">
                                @foreach ($allContacts as $index => $customer)
                                    @php
                                        $highlightClass =
                                            !$customer->is_read && $customer->hasMemo ? 'table-warning' : '';
                                    @endphp
                                    <tr class="{{ $highlightClass }}" data-id="{{ $customer->id }}">

                                    <tr class="{{ $highlightClass }}" data-id="{{ $customer->id }}">
                                        @if (auth()->user()->hasAnyRole(['admin', 'superadmin']))
                                            <td>
                                                <input type="checkbox" class="select-customer" value="{{ $customer->id }}">
                                            </td>
                                        @endif
    ...
    <script>
        $(document).ready(function() {
            $('.memo-btn').on('click', function(e) {
                    ...
                    $.post(url, {
                        ...
                        .done(function(res) {
                            if (res.success) {
                                ...
                                window.location.href = row.find('.memo-btn').attr(
                                'href'); // redirect to memo
                            }
                        })
                        .fail(function() {
                ...
                @else
                    window.location.href = row.find('.memo-btn').attr('href'); // normal redirect
                @endif
            });
